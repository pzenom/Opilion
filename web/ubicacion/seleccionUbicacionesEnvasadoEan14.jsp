...
<s:i18n name="ApplicationResources">
	<h3 class="handle">Sacar materias primas y envases para el proceso de envasado<span class="screenCode">SELE_UBICA_ENVA_EAN14</span></h3>
				...
				<table id="electedIngredientsTable" class="tabla" border="1" width="100%">
					<thead>
						<tr>
							...
							<th class="destaca">CANTIDAD SELECCIONAR (Kilos)</th>
							<th class="destaca">PROVEEDOR</th>
							<th class="destaca">FECHA ENTRADA</th>
						</tr>
					</thead>
					<tbody id="electedIngredientsBody">
						<s:iterator id="infomp" value="%{#session.items}">
							<s:if test="%{#infomp.idLinea == #infomp.idAnterior}">
							</s:if>
							<s:else>
								<tr id="escogerIngredientesNuevo<s:property value='idLinea' />">
									<td colspan="4" style="text-align:center; background-color: #CEF6CE !important;" >PRODUCTO: <s:property value="nombre" /></td>
									</tr>
							</s:else>
							<tr id="<s:property value='registroEntrada' />" class="ingrediente" style="font-size:14px;">
								...
								<td id="cantidadMP_<s:property value="registroEntrada" />" style="color:red;"><s:property value="teorica" /></td>
							...
							<s:iterator id="ubicacionesMp" value="%{#infomp.ubicaciones}" status="status">
								<tr style="font-size:14px;" id="<s:property value="registroEntrada" />_<s:property value="%{#status.count}" />" class="<s:property value="registroEntrada" />">
									...
									<td id="enHueco_<s:property value="registroEntrada" />_<s:property value="%{#status.count}" />" class="hueco" style="text-align: right; width: 90px;"><s:property value="cantidad" /></td>
									<td id="sacar_<s:property value="registroEntrada" />_<s:property value="%{#status.count}" />" style="text-align: right;">
										<s:if test="%{#status.count ==1}">
											<s:textfield id="aSacar_%{#infomp.registroEntrada}_%{#status.count}" cssClass="sacar" key="sacar_%{#ubicacionesMp.idHueco}_%{#ubicacionesMp.idUbicacion}_%{#infomp.registroEntrada}" onkeypress = "return validarNum(event)" value="%{#infomp.teorica}" cssStyle="width: 90px; text-align: right;" onblur="javascript:ajustarDecimal('aSacar_%{#infomp.registroEntrada}_%{#status.count}');" onkeypress="return validarSoloNumeros(event);" onchange="javascript:cantidadSacarActualiza('%{#infomp.registroEntrada}_%{#status.count}', true);" />
										</s:if>
										<s:else>
											<s:textfield id="aSacar_%{#infomp.registroEntrada}_%{#status.count}" cssClass="sacar" key="sacar_%{#ubicacionesMp.idHueco}_%{#ubicacionesMp.idUbicacion}_%{#infomp.registroEntrada}" onkeypress = "return validarNum(event)" value="0" cssStyle="width: 90px; text-align: right;" onblur="javascript:ajustarDecimal('aSacar_%{#infomp.registroEntrada}_%{#status.count}');" onkeypress="return validarSoloNumeros(event);" onchange="javascript:cantidadSacarActualiza('%{#infomp.registroEntrada}_%{#status.count}', true);" />
										</s:else>
									</td>
								</tr>
							</s:iterator>
							...
							<s:iterator id="ubicacionesEn" value="%{#infoen.ubicaciones}" status="status">
								...
								<s:else>
									...
									<td id="sacar_<s:property value="registroEntrada" />_<s:property value="idPalet" />_<s:property value="idHueco" />" class="ubicacion" style="text-align: right;">
										<s:if test="%{#status.count ==1}">
											<s:textfield id="aSacar_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}" cssClass="sacar" key="sacar_%{#ubicacionesEn.idHueco}_%{#ubicacionesEn.idUbicacion}_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}" value="%{#infoen.teorica}" cssStyle="width: 90px; text-align: right;" onblur="javascript:ajustarDecimal('aSacar_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}');" onkeypress="return validarSoloNumeros(event);" onchange="javascript:cantidadSacarActualiza('%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}', true);" />
										</s:if>
										<s:else>
											<s:textfield id="aSacar_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}" cssClass="sacar" key="sacar_%{#ubicacionesEn.idHueco}_%{#ubicacionesEn.idUbicacion}_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}" onkeypress = "return validarNum(event)" value="0" cssStyle="width: 90px; text-align: right;" onblur="javascript:ajustarDecimal('aSacar_%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}');" onkeypress="return validarSoloNumeros(event);" onchange="javascript:cantidadSacarActualiza('%{#infoen.registroEntrada}_%{#ubicacionesEn.idPalet}_%{#ubicacionesEn.idHueco}', true);" />
										</s:else>
									</td>
								</tr>
							</s:iterator>
