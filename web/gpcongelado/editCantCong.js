...
$(document).ready(function() {
	...
	$(".botonBotonera").hide();
	$("#bot_vuelve").attr("onclick", "javascript:nuevoGPCong();");
	$("#bot_vuelve").show();
	$("#bot_insertar").attr("onclick", "javascript:sacarUnidadesCongelar();");
	$("#bot_insertar").show();
});
...
function insertar(){
	...
	for (i = 0; i < seleccionados.length; i++){
		...
		if ($("#cantidad_" + id).val() == "" || $("#cantidad_" + id).val() == 0){
			$.msg("Introduzca la cantidad a congelar para la entrada " + id);
			flag = false;
			break;
		}
		//$.msg(id);
...
function sacarUnidadesCongelar(){
	//$.msg("holaa");
	$url = "UpdaRECongCant.action";
	...
	if (contador == 1){
		...
		if (contador == 0)
			...
			if (contador > 1)
				$.msg("Solo se permite congelar materias de la misma ubicación");
	}
}
