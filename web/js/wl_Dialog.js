...
$.alert = function (contenido, options) {
	...
	//fill the container
	container.html(contenido.replace(/\n/g, '<br>'));
	//display the dialog
...
$.msg = function (contenido, options) {
	...
	//stop if no contenido is set
	if(!contenido)return false;

	//count of displayed messages
	var count = container.data('msgcount');

	function getHTML(contenido, headline) {
		return '<div style="background: ' + options.background + ' !important; color: ' + options.colorLetra + ' !important;" class="msg-box"><h3 style="color: ' + options.colorLetra + ' !important;">' + (headline || '') + '</h3><a class="msg-close">close</a><div class="msg-contenido">' + contenido.replace('\n', '<br>') + '</div></div>';
	}

	function create() {
		msgbox = $(getHTML(contenido, options.header)),
		...
		if (count) {
			...
			if (!closeall.length) {
				...
				$('<div class="msg-box-close">Cerrar todos</div>').appendTo(container).fadeInSlide(options.fadeTime);
	...
	return {
		...
		setBody:function(html){
			msgbox.find('.msg-contenido').eq(0).html(html);
		},
