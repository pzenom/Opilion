		...
		if (!$this.data('wl_Widget')) {
			...
			$widgets.each(function () {
				var $widget = $(this),
					...
					$contenido = $widget.find('div').eq(0),
				...
				if(_opts.collapseable){
					...
					$handle.delegate('a.collapse', 'click.wl_Widget touchstart.wl_Widget', function (event) {
						var _opts = $widget.data('wl_Widget') || _opts,
							_contenido = $widget.find('div').eq(0);
						
						if (_contenido.is(':hidden')) {
							
							//expand hidden contenido
							_contenido.slideDown(100, function () {
							...
							//hide contenido
							$contenido.slideUp(100, function () {
				...
				$handle.delegate('a.reload', 'click.wl_Widget touchstart.wl_Widget', function (event) {
					...
					_contenido.height($contenido.height());
					
					//removecontenido and replace it with a loading information
					if (_opts.removecontenido) {
						_contenido.html(_opts.text.loading);
					}
					_contenido.load(_opts.load, function (response, status, xhr) {
						$widget.removeClass('loading');
						_contenido.height('auto');
						
						//error occured
						if (status == "error") {
							_contenido.html(xhr.status + " " + xhr.statusText);
						}
			...
			var $maincontenido = $('#contenido');

			//save the total count of widgets
			if (!$maincontenido.data('wl_Widget')) {
				$maincontenido.data('wl_Widget', {
			...
			if ($maincontenido.data('wl_Widget').currentid++ >= $maincontenido.data('wl_Widget').containercount) {
