$(document).ready(function () {
	...
	(function ($) {
		...
		function Wysiwyg() {
			this.controls = {
				...
				html: {
					...
					exec: function () {
						...
						if (this.viewHTML) {
							this.setcontenido(this.original.value);
			...
			this.defaults = {
				html: '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>INITIAL_contenido</body></html>',
				debug: false,
				controls: {},
				css: {},
				events: {},
				...
				formWidth: 440,
				iFrameClass: null,
				initialcontenido: "<p>Initial contenido</p>",
			...
			this.ui.focus = function () {
				...
				self.editor.get(0).contenidoWindow.focus();
				return self;
			};
			...
			this.increaseFontSize = function () {
				if ($.browser.mozilla || $.browser.opera) {
					...
					this.getInternalRange().surroundcontenidos(newNode);
				} else {
			...
			this.getcontenido = function () {
				...
				return this.events.filter('getcontenido', this.editorDoc.body.innerHTML);
			};
			 ...
			 * Example:
			 * 
			 * $("#editor").getWysiwyg().events.bind("getcontenido", function (orig) {
			 *     return "<div id='contenido'>"+orgi+"</div>";
			 * });
			 * 
			 * This makes it so that when ever getcontenido is called, it is wrapped in a div#contenido.
			 */
			this.events = {
				_events : {},
			...
			this.getInternalSelection = function () {
				...
				if (this.editor.get(0).contenidoWindow) {
					if (this.editor.get(0).contenidoWindow.getSelection) {
						return this.editor.get(0).contenidoWindow.getSelection();
					}
					if (this.editor.get(0).contenidoWindow.selection) {
						return this.editor.get(0).contenidoWindow.selection;
					}
				}
			...
			this.ui.grow = function () {
				var self = this.self,
					...
					innerHeight = $.browser.msie ? innerBody[0].scrollHeight : innerBody.height() + 2 + 20, // 2 - borders, 20 - to prevent contenido jumping on grow
			...
			this.init = function (element, options) {
				 ...
				 */
				this.initialcontenido = $(element).val();
				this.ui.initFrame();
			...
			this.ui.initFrame = function () {
				...
				self.editorDoc.write(
					self.options.html
						 ...
						 */
						.replace(/INITIAL_contenido/, function () { return self.wrapInitialcontenido(); })
				);
				...
				$(self.editorDoc).keydown(function (event) {
					var emptycontenidoRegex;
					if (event.keyCode === 8) { // backspace
						emptycontenidoRegex = /^<([\w]+)[^>]*>(<br\/?>)?<\/\1>$/;
						if (emptycontenidoRegex.test(self.getcontenido())) { // if contenido is empty
				...
				if (self.options.autoGrow) {
					...
					// fix when contenido height > textarea height
					self.ui.grow();
				}
				...
				if (self.initialcontenido.length === 0) {
					if ("function" === typeof (self.options.initialcontenido)) {
						self.setcontenido(self.options.initialcontenido());
					} else {
						self.setcontenido(self.options.initialcontenido);
					}
				}
				...
				if (self.options.xhtml5 && self.options.unicode) {
					...
					self.events.bind("getcontenido", function (text) {
			...
			this.innerDocument = function () {
				...
				if (element.nodeName.toLowerCase() === "iframe") {
					if (element.contenidoDocument) {				// Gecko
						return element.contenidoDocument;
					} else if (element.contenidoWindow) {			// IE
						return element.contenidoWindow.document;
					}
					...
					/*
					 ...
					 : element.contenidoWindow.document // contenidoDocument;
					 */
				}
			...
			this.resetFunction = function () {
				this.setcontenido(this.initialcontenido);
			};
	
			this.savecontenido = function () {
				if (this.viewHTML)
				{
					return; // no need
				}
				if (this.original) {
					var contenido, newcontenido;
					...
					if (this.options.replaceDivWithP) {
						newcontenido = $("<div/>").addClass("temp").append(contenido);
	
						newcontenido.children("div").each(function () {
						...
						contenido = newcontenido.html();
					}
	
					$(this.original).val(contenido);
			...
			this.setcontenido = function (newcontenido) {
				this.editorDoc.body.innerHTML = newcontenido;
				this.savecontenido();
			...
			this.wrapInitialcontenido = function () {
				var contenido = this.initialcontenido,
					found = contenido.match(/<\/?p>/gi);
		...
		$.wysiwyg = {
			...
			init: function (object, options) {
				return object.each(function () {
					...
					// :4fun:
					// remove this textarea validation and change line in this.savecontenido function
					// $(this.original).val(contenido); to $(this.original).html(contenido);
			...
			setcontenido: function (object, newcontenido) {
				return object.each(function () {
					var oWysiwyg = $(this).data("wysiwyg");
					...
					oWysiwyg.setcontenido(newcontenido);
				});
			},
			...
			if ($.fn.dialog) {
				...
				if (a.self) {
					...
					if ((selection && selection.length > 0) || img) {
						...
						window.$.msg(Wysiwyg.options.messages.nonSelection);
					}
				}
	...
	$.wysiwyg.createLink = function (object, url) {
		return object.each(function () {
			...
			if (selection && selection.length > 0) {
				...
				window.$.msg(oWysiwyg.options.messages.nonSelection);
			}
		});
	};
})(jQuery);
				...
				$("form.wysiwyg").dialog({
					...
					open: function (ev, ui) {
						...
						if (range) {
							...
							$.msg("You must select some elements before you can wrap them.");
							$this.dialog("close");
							return 0;
						}
						...
						if (common.parent(".wysiwygCssWrapper").length) {
							$.msg(common.parent(".wysiwygCssWrapper").get(0).nodeName.toLowerCase());
