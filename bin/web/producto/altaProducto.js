...
$(document).ready(function() {
	...
	/* Configuramos el boton para la subida de la imagen */
});
...
function tipoEan(tProd){
	...
	$("#pestaniaImagen").show();
	$("#bot_insertar").attr("onclick", "javascript:registraProducto();");
	$("#bot_insertar").show();
...
function cargaFamilia(){
	//$.msg("hola");
	var selected = document.getElementById("idFamilia").selectedIndex;
	//$.msg("Indice de familia seleccionada: " + selected);
	//En y las opciones
	var y = document.getElementsByTagName("option");
	//$.msg(y[1].text);
	//Cargar grupos en funcion de la familia "selected" (Tabla 
}

function cargaGrupo(){
	//$.msg("hola");
	var selected = document.getElementById("idGrupo").selectedIndex;
	//$.msg("Indice de grupo seleccionado: " + selected);
	//En y las opciones
	var y = document.getElementsByTagName("option");
	//$.msg(y[1].text);
}
...
function cargaCategoria(){
	//$.msg("hola");
	var selected = document.getElementById("idCategoria").selectedIndex;
	//$.msg("Indice de grupo seleccionado: " + selected);
	//En y las opciones
	var y = document.getElementsByTagName("option");
	//$.msg(y[1].text);
		...
		if (!$("#CANTIDAD_Item_" + idProducto).length){
			...
			nuevaFila.innerHTML =
				...
				'<td style="width: 25px;">' +
					...
					'</a>' +
				'</td>';
			document.getElementById('tabla_Items_body').appendChild(nuevaFila);
		}
	}
}
...
function cargaEnvase(){
	...
	//TODO: Eliminar envase del select
}
...
function masComponentes(){
	if ($("#div_compuesto").is (':visible')){
		$("#div_compuesto").hide("slow");
	}else{
		$("#div_compuesto").show("slow");
	}
}
	...
	if (flag){
		if (tipo == 13){//Definimos un item
			...
			if (cantidadesIngredientes.length == 0){
				flag = false;
				$.msg("No se puede definir un EAN13 sin ingredientes asociados");
			}else{
				for (i = 0; i < cantidadesIngredientes.length; i++){
					...
					if (parseFloat(cantidad) == 0){
						flag = false;
						$.msg("La cantidad de cada materia prima asociada al producto debe ser mayor que cero.");
						break;
					}
				}
			}
	...
	if (flag){
		if ($("#text_nombre").val() == ""){
			...
			if (cuantosGruposSeleccionados == 0){
				...
				$.confirm("&#191Registrar producto?",
					function(){
						$.msg("Registrando el producto...");
						var $url = "RegistroNuevoProducto.action?tipo=" + tipo + "&";
						//1. Datos generales del producto
						$url += "nombre=" + $("#text_nombre").val() + "&descripcion=" + $("#text_descripcion").val() +
							"&stock=" + $("#text_stock").val() + "&stockMinimo=" + $("#text_stockMinimo").val() +
							"&codigoEan=" + $("#text_ean").val() + "&peso=" + $("#text_peso").val() + "&caducidad=" + $("#text_caducidad").val() +
							"&precio=" + $("#text_precio").val() + "&idImpuesto=" + $("#dropdown_impuestos").val() +
							"&idCategoria=" + $("#dropdown_categorias").val();
						...
						if (tipo == 14){
							items = $("#tabla_Items_body").children();
					...
					function(){
						$.msg("Cancelada la insercion del nuevo producto");
					}
				);
			}
		}
