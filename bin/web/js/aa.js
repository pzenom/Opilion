...
function AjaxAnywhere() {
    ...
    this.delayBeforecontenidoUpdate = true;
...
AjaxAnywhere.prototype.findForm = function () {
    ...
    if (typeof form != "object")
        $.msg("AjaxAnywhere error: Form with name [" + this.formName + "] not found");
    return form;
}
...
AjaxAnywhere.prototype.submitAJAX = function(additionalPostData, submitButton) {
    ...
    this.req.setRequestHeader("contenido-Type", "application/x-www-form-urlencoded;charset=UTF-8");
...
/**
* Handle wrong contenido type. 
*/
AjaxAnywhere.prototype.handleWrongcontenidoType = function() 
{
	$.msg("AjaxAnywhere error : contenido-type in not text/xml : [" + this.req.getResponseHeader('contenido-type') + "]");
}
...
AjaxAnywhere.prototype.callback = function() {
    ...
    if (this.req.readyState == 4) {
        ...
        if (this.req.status == 200) {
            ...
            if (this.req.getResponseHeader('contenido-type').toLowerCase().substring(0, 8) != 'text/xml') {
                this.handleWrongcontenidoType();
                return;
            }
            ...
            if (docs.length != 0) {
                var newcontenido = docs[0].firstChild.data;

                //cleanup ressources
                delete this.req;

                document.close();
                document.write(newcontenido);
                document.close();
            }
            ...
            if (images.length != 0) {
                ...
                if (this.delayBeforecontenidoUpdate) {
...
/**
* Some browsers (notably IE) do not load images from thier cache when contenido is updated using
* innerHTML. As a result, each image is re-requested from the server even though the image exists
* in the cache. To work around this issue, AjaxAnywhere preloads images present in the new contenido
...
AjaxAnywhere.prototype.setDelayBeforeLoad = function (isDelay) {
    this.delayBeforecontenidoUpdate = isDelay;
}
...
AjaxAnywhere.prototype.isDelayBeforeLoad = function () {
    return this.delayBeforecontenidoUpdate;
}
...
* by this function. The default implementation is $.msg(stackTrace);
...
AjaxAnywhere.prototype.handleHttpErrorCode = function(code) {
    ...
    if (details) {
        ...
        if (win != null) {
            ...
            $.msg("Please, disable your pop-up blocker for this site first.");
        }
    }
}
...
AjaxAnywhere.prototype.handlePrevousRequestAborted = function() {
    //$.msg("AjaxAnywhere default error handler. INFO: previous AJAX request dropped")
}
