...
<s:i18n name="ApplicationResources">
	<h3 class="handle"><s:text name="gestionProcesoEnvasado.bienvenida" />: <s:property value="#session.orden" /><span class="screenCode">GESENVA01_EAN14</span></h3>
			...
			<div id="infoProceso">
				...
				<s:form id="gestionaEnvasado" name="gestionaEnvasado" action="" validate="true">
					<table id="electedIngredientsTable" class="tabla" border="1" width="100%">
						<thead>
							<tr>
								<th class="destaca" rowspan="2" style="width: 100px;">PRODUCTO</th>
							...
							<s:iterator id="infoprod" value="%{#session.items}">
								...
								<s:else>
									<tr id="escogerIngredientesNuevo<s:property value='idLinea' />">
										<td class="verde" colspan="9" style="text-align:center; background-color: #CEF6CE !important;" >PRODUCTO: <s:property value="nombre" /></td>
									</tr>
								</s:else>
								<tr id="<s:property value='registroEntrada' />" class="ingrediente">
									<td class="entradaProducto"><s:property value="registroEntrada" /></td>
									<td id="ubicaPROD_<s:property value="registroEntrada" />"><s:property value="refUbicacion" /></td>
									<td id="cantidadPROD_<s:property value="registroEntrada" />_<s:property value="idUbicacion" />" style="text-align: right; display: none;"><s:property value="cantidadUbicacion" /></td>
									<td id="realPROD<s:property value="registroEntrada" />" onkeyup="javascript:checkCantidadIngrediente('<s:property value="registroEntrada" />')"><s:textfield cssStyle="text-align:right;font-size:20px;width:60px;" value="%{real}" cssClass="inputRellenar" name="real_prod_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" disabled="true" /></td>
									<td id="mermasPROD_<s:property value="registroEntrada" />_<s:property value="idUbicacion" />"><s:textfield cssStyle="text-align:right; font-size:20px;width:60px;" cssClass="inputRellenar" value="%{mermas}" name="mermas_prod_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" disabled="true" /></td>
									<td id="sobra_<s:property value="registroEntrada" />_<s:property value="idUbicacion" />"><s:textfield cssStyle="text-align:right; font-size:20px;width:60px;" cssClass="inputRellenar" value="0" name="sobra_prod_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" id="sobra_prod_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" disabled="true"/></td>
									<td id="fecha_<s:property value="registroEntrada" />"><s:property value="fecha" /></td>
									<td id="fechaCad_<s:property value="registroEntrada" />"><s:property value="fechaCaducidad" /></td>
									<td id="fechaPROD_<s:property value="registroEntrada" />" style="display:none;"><s:property value="fecha" /></td>	
									<td id="fechaCaducidadPROD_<s:property value="registroEntrada" />" style="display:none;"><s:property value="fechaCaducidad" /></td>
									...
									<td class="ubicacionesDevolver" id="ubicacionDevolver_<s:property value="registroEntrada"/>" style="visibility: hidden;">
										<s:textfield cssClass="inputReubicarPROD" cssStyle="text-align:right; font-size:20px;" onclick = "javascript:reubicar('%{#infoprod.registroEntrada}', 'PROD', '%{#infoprod.idPalet}', '%{#infoprod.idUbicacion}');" value="%{#infoprod.refUbicacionNueva}" name="reubicar_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" id="reubicar_%{#infoprod.registroEntrada}_%{#infoprod.idUbicacion}" />
									</td>
					...
					<table id="electedEnvasesTable" class="tabla" border="1" width="100%">
						...
						<tbody id="electedEnvasesBody" >
							<s:iterator id="infoen" value="%{#session.envases}">
								...
								<s:else>
									<tr id="escogerIngredientesNuevo<s:property value='idLinea' />">
										<td colspan="9" style="text-align:center; background-color: #CEF6CE !important;" >ENVASE AGRUPACI&Oacute;N: <s:property value="nombre" /></td>
								</tr>
								</s:else>
