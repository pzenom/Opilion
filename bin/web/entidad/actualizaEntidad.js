...
$(document).ready(function() {
	...
	autoSaltaCampos();
	//ocultarBreadcrumbs();
	//Calculamos los valores de var siguienteLinea, siguienteLineaContacto, siguienteLineaTfno, siguienteLineaEmail, siguienteLineaFormaPago
	siguienteLineaContacto = parseFloat($("#tbodyDirsContacto").children().length) + 1;
	siguienteLineaTfno = parseFloat($("#tbodyTfnos").children().length) + 1;
	siguienteLineaEmail = parseFloat($("#tbodyEmails").children().length) + 1;
	siguienteLineaFormaPago = parseFloat($("#tbodyCuentasBanco").children().length) + 1;
	//Miramos los roles de la entidad
	roles = $(".roles");
	for (i = 0; i < roles.length; i++){
		...
		rol = $("#" + idRol).val();
		if (rol == 1){
			if ($("#bot_cliente").hasClass('btActivado') == false){
				entidadCliente();
			}
		}else{
			if (rol == 2){
				if ($("#bot_proveedor").hasClass('btActivado') == false){
					entidadProveedor();
				}
			}
		}
	...
	//Autonomo o empresa?
	autonomoEmpresa = $("#tipoPersona").val();
	if (autonomoEmpresa == 'A'){
		entidadAutonomo();
	}else{
		if (autonomoEmpresa == 'E'){
			entidadEmpresa();
		}
	}//FIN. Autonomo o empresa cargados
	/* Configuramos la botonera */
	$(".botonBotonera").hide();
	$("#bot_actualizar").attr("onclick", "javascript:actualizaEntidad();");
	$("#bot_actualizar").show();
	...
	//alert(3);
	//Mostramos/Ocultamos la parte de las formas de pago que se refieren a pago a 0 dias
	spans = $('.spanDiasFormaPago').get();
	for (i = 0; i < spans.length; i++){
		...
		if (parseFloat(dias) > 0){
			$('#spanFormaPagoDias_' + frag[1]).show();
		}
		//spanFormaPagoDias_X
	}
	exportable = $('#exportable').val();
	//alert(exportable);
	if (exportable == 'S' && !$("#checkExportableAutoventa").is(':checked')){
		$('#checkExportableAutoventa').trigger('click');
	}
});
...
function guardarDireccion(){
	...
	if ($("#dropdown_tiposDire").val() == 0)
		...
		//$.msg("hallo");
		nuevaFila.innerHTML =
			'<input type="hidden" id="lineaContacto_' + siguienteLineaContacto + '" name="lineaContacto_' + siguienteLineaContacto + '" value="0" />' +
...
function addTelefono(){
	if ($("#tipo_tfno").val() == 0){
		...
		if ($("#text_numero").val() == "")
			...
			// Crear la fila
			nuevaFila.innerHTML =
				'<input type="hidden" id="lineaTfno_' + siguienteLineaTfno + '" name="lineaTfno_' + siguienteLineaTfno + '" value="0" />' +
	...
	if ($('#text_diasFormaPago').val() > 0 && $('#dropdown_formaPagoDesde').val() == 0){
		...
		if ($("#dropdown_formasPago").val() < 1){
			...
			if (!($("#cuentaAsociada").is(':checked')) || ($("#cuentaAsociada").is(':checked') && validar())){
				...
				// Crear la fila
				nuevaFila.innerHTML =
					'<input type="hidden" id="lineaFormaPago_' + siguienteLineaFormaPago + '" name="lineaFormaPago_' + siguienteLineaFormaPago + '" value="0" />' +
				...
				$("#tipoFormaPago_" + siguienteLineaFormaPago).val($("#dropdown_formasPago").val());
...
function addNuevoEmail(){
	if ($("#dropdown_tipoEmail").val() == 0){
		...
		if ($("#text_mail").val() == "")
			...
			// Crear la fila
			nuevaFila.innerHTML =
				'<input type="hidden" id="lineaEmail_' + siguienteLineaEmail + '" name="lineaEmail_' + siguienteLineaEmail + '" value="0" />' +
...
function editaFormaPago(numLinea){
	//alert($('#tipoFormaPago_' + numLinea).val());
	...
	//alert($('#diasFormaPago_' + numLinea).text());
	//alert(0);
	if (parseFloat($('#diasFormaPago_' + numLinea).text().trim()) > 0){
		//alert(1);
		//$('#contenedorFormaPagoDias').show();
		//$('#spanFormaPagoDias').show();
	}
...
function actualizaEntidad() {
	...
	if (flag){
		...
		if (($("#bot_autonomo").hasClass('btActivado') == false) && ($("#bot_empresa").hasClass('btActivado') == false)){
			...
			$.confirm("&#191Confirma que desea guardar los cambios?",
				function(){
					...
					for (i = 0; i < direcciones.length; i++){
						...
						frag = id.split("_");
						indice = frag[1];
						urlDireccion = "lineaContacto_" + indice + "=" + $("#lineaContacto_" + indice).val() + "&provinciaContacto_" + indice + "=" + $("#provinciaContacto_" + indice).val() +
					...
					for (i = 0; i < telefonos.length; i++){
						...
						frag = id.split("_");
						indice = frag[1];
						urlTelefono = "lineaTfno_" + indice + "=" + $("#lineaTfno_" + indice).val() + "&numeroTelefono_" + indice + "=" + $("#numeroTelefono_" + indice).val() +
					...
					for (i = 0; i < emails.length; i++){
						...
						frag = id.split("_");
						indice = frag[1];
						urlEmail = "lineaEmail_" + indice + "=" + $("#lineaEmail_" + indice).val() + "&direccionEmail_" + indice + "=" + $("#direccionEmail_" + indice).val() +
					...
					for (i = 0; i < formasDePago.length; i++){
						...
						frag = id.split("_");
						indice = frag[1];
						cadenaDiasFormaPago = "0";
						if ($("#diasFormaPago_" + indice).text() != "")
							cadenaDiasFormaPago = $("#diasFormaPago_" + indice).text().trim();
						urlFormaDePago = "lineaFormaPago_" + indice + "=" + $("#lineaFormaPago_" + indice).val() + "&formaPago_" + indice + "=" + $("#tipoFormaPago_" + indice).val() +
							"&diasFormaPago_" + indice + "=" + cadenaDiasFormaPago +
					...
					//Observaciones
					var observaciones = tinyMCE.get('txt_editaEntidad_observaciones').getContent();
					//alert(observaciones);
					...
					var $url = "ActualizarEntidad.action" + urlFINAL;
					//alert($url);
					$.ajax({
						type: 'POST',
						url: $url,
...
function autoSaltaCampos(){
	$('#cuenta_iban').keyup(function() {
		if($('#cuenta_iban').val().length>3) {
			$('#cuenta_banco').focus();
		}
	});
	$('#cuenta_banco').keyup(function() {
		if($('#cuenta_banco').val().length>3) {
			$('#cuenta_ofici').focus();
			muestraBanco();
		}
	});
	$('#cuenta_ofici').keyup(function() {
		if($('#cuenta_ofici').val().length>3) {
			$('#cuenta_contr').focus();	
		}
	});
	$('#cuenta_contr').keyup(function() {
		if($('#cuenta_contr').val().length>1) {
			$('#cuenta_numer').focus();	
		}
	});
	$('#cuenta_numer').keyup(function() {
		if($('#cuenta_numer').val().length>9) {	
