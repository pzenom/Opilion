...
public class InseRECribAction extends ActionSupport implements
	org.apache.struts2.interceptor.ServletRequestAware, ModelDriven<Object>, SessionAware{
	
	private HttpServletRequest request;
	private Map session;
	private List listCateCrib; 
	private RegistroEntrada entry = new RegistroEntrada();
	private GestionProduccion gprod = new GestionProduccion();
	private String url;
	public void setListCateCrib(List listCateCrib) {this.listCateCrib = listCateCrib;}
	public List getListCateCrib() {return listCateCrib;}

	//@Override
	public void setServletRequest(HttpServletRequest request) {
		System.out.println("InseGPCribAction Action...");
	...
	/**
	 * 1. Indica la cantidad a cribadoar
	...
	public String execute() throws Exception {
		String proceso = "Cribado";//"CribadoM";
		HashMap mapaCantRE = new HashMap();
		List listRECong = new ArrayList();
		//HashMap categorias = (HashMap) session.get("listCateCrib");
		String[] categorias = request.getParameterValues("listCateCrib");
		String[] cantity = request.getParameterValues("cantidad");
		//Agrega las cantidades a las categor�as generadas
		...
		//Recupera el c�digo del RE Padre
		...
		//Crea los nuevos SRE basados en las categor�as y cantidades guardadas en el map
		String orden = new GestionRegistroEntradaHelper().
			addRegistrosSubEntradaProceso(entry, mapaCantRE, proceso, entry.getSaldo());
		setUrl("UbicarProceso.action?tipoProceso=Cribado&proceso=" + orden);
		request.getSession().setAttribute("tipoProceso", "Cribado");
		request.getSession().setAttribute("proceso", orden);
		request.setAttribute("tipoProceso", "Cribado");
	...
	private void addCategoriasCantidades(HashMap mapaCantRE, String[] categorias, String[] cantity) {
		...
		//Cont = numero de clases cribadoadas
		...
		for (int i=0;i<cant.length;i++){
			Iterator iter = (Iterator) listCateCrib.iterator();
			while ( iter.hasNext()){
				String ing = (String)iter.next();
				ing = ing.substring(0, ing.length()-1);
				System.out.println("categoria a�adida a la lista: "+ing);
		...
		for (Iterator it = mapaCantRE.keySet().iterator(); it.hasNext();) { 
            ...
            System.out.println("Map creado. CATEGORIA: "+ s + " -> CANTIDAD: " + s1);
		}
	}
