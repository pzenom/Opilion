...
public class InseRESeleAction extends ActionSupport implements
	org.apache.struts2.interceptor.ServletRequestAware, ModelDriven<Object>, SessionAware{
	
	private HttpServletRequest request;
	private Map session;
	private List listCateSele; 
	private RegistroEntrada entry = new RegistroEntrada();
	private GestionProduccion gprod = new GestionProduccion();
	private String url;
	public void setListCateSele(List listCateSele) {this.listCateSele = listCateSele;}
	public List getListCateSele() {return listCateSele;}

	//@Override
	public void setServletRequest(HttpServletRequest request) {
		System.out.println("InseGPSeleAction Action...");
	...
	public String execute() throws Exception {
		String proceso = "SeleccionM";
		HashMap mapaCantRE = new HashMap();
		List listRECong = new ArrayList();
		//HashMap categorias = (HashMap) session.get("listCateSele");
		String[] categorias = request.getParameterValues("listCateSele");
		...
		setUrl("UbicarProceso.action?tipoProceso=Seleccion&proceso=" + orden);
		request.getSession().setAttribute("tipoProceso", "Seleccion");
		request.getSession().setAttribute("proceso", orden);
		request.setAttribute("tipoProceso", "Seleccion");
	...
	private void addCategoriasCantidades(HashMap mapaCantRE, String[] categorias, String[] cantity) {
		...
		for (int i=0;i<cant.length;i++){
			Iterator iter = (Iterator) listCateSele.iterator();
			while ( iter.hasNext()){
