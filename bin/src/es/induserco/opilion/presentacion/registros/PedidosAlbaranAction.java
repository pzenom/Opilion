...
public class PedidosAlbaranAction extends ActionSupport implements
org.apache.struts2.interceptor.ServletRequestAware, ModelDriven {
	...
	public void setServletRequest(HttpServletRequest request) {
		System.out.println("PedidosAlbaranAction Action...");
	...
	public Object getModel() {		
 	...
 	public String execute() throws IOException {
		try {
			//1. Leemos los identificadores de los pedidos que vamos a unir en el albar�n
			Map <String, String[]> parametros = request.getParameterMap();
			String nombreParametro = "";
			List<String> codigosPedidos = new ArrayList<String>();
			...
			while(iterator.hasNext()) {
				...
				if (nombreParametro.indexOf("codigoPedido_") > -1){
					...
					codigosPedidos.add(cod);
				}else{
					if (nombreParametro.indexOf("fechaEntrega") > -1){
						//1.1 Leemos la nueva fecha de entrega para el albar�n
			...
			String codigo = new GestionRegistroSalidaHelper().unirPedidos(codigosPedidos, dtm);//Nuevo bgmNum
			...
			if (request.getSession().getAttribute("pesoNetoTotal") != null)
				albaranTotales.setPesoNetoTotal(Double.parseDouble(request.getSession().getAttribute("pesoNetoTotal").toString()));
