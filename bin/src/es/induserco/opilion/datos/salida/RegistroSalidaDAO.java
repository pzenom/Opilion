...
public class RegistroSalidaDAO extends DatabaseConfig implements IRegistroSalidaDataService {
	...
	/**
	 * A partir del c�digo EAN de un producto, obtenemos su descripci�n
	 *
	 * @author Induserco, Andr�s (27/04/2011)
	 * @since 0.3
	 * @param EAN
	...
	/**
	 * Inserta un Albar�n Simple o a partir de una Orden de EDIFACT.
	 *
	 * @param albaran
	 *            the albaran
	 * @return the boolean
	...
	public Boolean addAlbaran(Albaran albaran) throws Exception {
		...
		try {
			...
			stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_UPDATABLE);
			res = stmt.executeUpdate(insertString);
			// Si el pedido est� compuesto por lineas de otros pedidos, marcamos
	...
	public boolean addFactura(String codigoAlbaran, Factura fact) throws Exception {
		...
		try {
			...
			if (res1 == 1 && res2 == 1 && res3 == 1) {
				// System.out.println("factura_detalle INSERTADO ahora actualizo el estado de ese albar�n");
	...
	public Boolean updateEstadoAlbaran(String codigoAlbaran) throws Exception {
		...
		try {
			...
			if (res1 == 1) {
				// System.out.println("Estado de albar�n actualizado INSERTADO");
				resultado = true;
			}
			return resultado;
		} catch (Exception e) {
	...
	/**
	 * Obtiene el detalle del albar�n para poder generar una factura.
	 *
	 * @param codigoAlbaran
	 *            the codigo albaran
	 * @param fecha
	...
	public Vector<Albaran> getAlbaranes(Albaran albaran, int filtro) throws Exception {
		...
		try {
			...
			if (albaran != null) {
				// Si viene vacio es porque se trata de un albar�n simle
	...
	/**
	 * Tabla accion. Acci�n que muestra las facturas en el escritorio, id = 14
	 */
	//@Override
	...
	/**
	 * Obtiene el encabezado del albar�n para poder generar una factura.
	 *
	 * @param codigoAlbaran
	 *            the codigo albaran
	...
	/**
	 * Obtiene los RS asociados a la l�nea del albar�n Cuando se tiene una orden
	 * previa.
	 *
	 * @param codigoAlbaran
	 *            the codigo albaran
	...
	public Vector<Object> getRSLineaAlbaranTotal(String codigoAlbaran,
		...
		try {
			con = bddConexion();
			// obtener totales de la l�nea del detalle del albar�n
	...
	public Vector<RegistroSalida> getRegistroSalidas(String lote,
		...
		try {
			...
			while (rs.next()) {
				...
				// A�adimos al Vector de resultado
				resultado.add(salida);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * @author Induserco, Andr�s (19/04/2011) - (26.10.2011)
	 * @since 0.2
	 */
	//@Override
	...
	/**
	 * @author Induserco, Andr�s (19/04/2011)
	 */
	//@Override
	...
	/**
	 * @author Induserco, Andr�s (26/04/2011)
	 * @since 0.3
	 */
	//@Override
	...
	/**
	 * M�todo que devuelve el albaran para el reporte
	 */
	//@Override
		...
		try {
			...
			while (rs.next()) {
				...
				if (frag.length > 1){
					...
					//4 primeros: IBAN
					//Mostrar los 4 siguientes (banco) y los 4 �ltimos
					if (cuentaBanco != null && cuentaBanco.length() == 24){
						...
						//Tenemos que buscar el n�mero de cuenta en la cadena y sustituirlo por cuentaOculta
				...
				while (rs2.next()) {
					...
					if (salida.getLote().charAt(0) == 'E') {
						...
						//Comprobamos si el producto final es una agrupaci�n o no
						consulta =
							" SELECT * " +
					...
					if (granel){
						...
						if (noAgrupacion){
							//Si es un producto (no granel), y no es una agrupaci�n, bultos = unidades
	...
	public Factura getFactura(String codigo) throws Exception {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				// Factura normal, asociada a albar�n
				...
				// Factura libre, no est� asociada a ning�n albar�n
	...
	public Albaran unirAlbaranes(List<String> codigosAlbaranes, DTM dtm) throws Exception {
		...
		try {
			...
			//Buscamos las lineas del albar�n
			String QryRegistrosSalida =
	...
	public char comprobarLoteLeido(Lote lote) throws Exception {
		...
		try {
			...
			if (lote.getLote().charAt(0) == '0') {
				// Comprobamos primero si en el proceso con lote lote.getLote() se agrup�
				qry =
	...
	public Vector<Lote> getLotesProducto(long idProducto) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				//Si el lote es de un proceso de envasado de agrupaci�n, no cuenta, porque agrupamos, no tenemos el EAN13
				String consulta =
					" SELECT idAgrupacion " +
	...
	public void actualizarAlbaran(Albaran albaran) throws Exception {
		...
		try {
			...
			stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			stmt.executeUpdate(insertString);
			// Actualizamos cada linea del albar�n
			//TODO
	 ...
	 * @return Vehiculo, incluyendo su carga y su c�digo QR
	 * @throws Exception
	 */
	//@Override
	public Comercial getLanzaderaPedidos(long idComercial) throws Exception{
		...
		try {
			con = bddConexion();
			// Para el idComercial, mirar cuando fue la �ltima sincronizaci�n
			...
			// Mirar si despues de la �ltima sincronizaci�n hubo alguna lanzadera para el comercial
			qry =
				" SELECT log.fecha " +
			...
			//TODO Miramos que id tendr� la lanzadera que vamos a generar. El log lanzadera lo insertamos m�s adelante (insertaloglanzadera)
			qry =
			...
			for (int i = 0; i < cuantasHojas; i++){
				...
				//Recorre 10 pedidos que ir�n en esta pagina
	...
	public Lanzadera insertaLogLanzadera(Lanzadera l) throws Exception{
		...
		try {
			...
			//Buscamos totalLanzaderas (En este caso tendr� el mismo valor que lanzaderaNumero)
			qry =
