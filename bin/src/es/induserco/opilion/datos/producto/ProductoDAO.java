...
/**
 * Producto DAO
 * @author Induserco, Andr�s (29/04/2011)
 * @version 0.3
 */
public class ProductoDAO extends DatabaseConfig implements IProductoDataService {
	...
	public Boolean addProducto(Producto producto, boolean idEspecifico, long id) throws Exception {
		...
		try {
			...
			/*
			//Si insertamos un producto con el id espec�fico, es que lo estamos actualizando
			if (idEspecifico){
				//Actualizamos el hist�rico del producto
			}*/
			return resultado;	
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean setMateriasPrimasProducto(long idProducto, Vector<double[]> materias) throws Exception{
		...
		try{
			...
			for (int i = 0; i < materias.size() ; i++){
				//Cada posici�n  vector recibido es una dupla: idMateria, cantidadMateria
				...
				if (res == 1)
					resultado = true;
				//Insertamos tambien en el hist�rico del producto
				insertString = 
	...
	/**
	 * @author Induserco, Andr�s (13/04/2011)
	...
	/**
	 * @author Induserco, Andr�s (11/05/2011)
	 ...
	 * @author Induserco, Andr�s (12/04/2011)
	 * @throws Exception Si falla la actualizaci�n
	 * @since 0.2
	 */
	//@Override
    ...
    /*
     * Obtenemos los productos q forman una agrupaci�n
     */
    //@Override
	 ...
	 * Se llama a este m�todo cuando mostramos un producto para mostrar/editar, y cargamos los envases asociados al producto
	 * @param id Id del producto
	 * @param ean C�digo ean del producto
	...
	public boolean eliminarProducto(long id) throws Exception {
		//Desde aqu�, eliminar el producto de las tablas: producto, pro_compuesto, pro_materiaprima, pro_envase
	...
	public void addStockProducto(String lote, double cantidad) throws Exception{
		// System.out.println("A�adimos stock al producto");
		try {
			con = bddConexion();
			long idProducto = 0;
			ResultSet rs = null;
	...
	/**
	 * A partir de un idProducto, obtiene todos los envases donde el producto podr�a ser agrupado
	...
	/**
	 * A partir de un idProducto, id de un EAN13, obtiene todas las agrupaciones que se pueden hacer con �l
	...
	public String validarLote(String lote) throws Exception{
		...
		try {
			...
			if (lote.charAt(0) == '0'){
				//Comprobar si existe en el hist�rico de envasados
				String Qry =
				...
				if (lote.charAt(0) == 'E'){
					//Comprobar si existe en el hist�rico de entradas
					String Qry =
						" SELECT re.idEntrada " +
						" FROM registroentrada re " +
						" WHERE re.codigoEntrada = '" + lote + "'; ";
