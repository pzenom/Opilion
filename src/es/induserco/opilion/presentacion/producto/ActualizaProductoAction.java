...
public class ActualizaProductoAction extends ActionSupport implements
	...
	public String execute() throws Exception {
		...
		/*
		 * 1. Borrar toda la información que hay del producto (Tablas: producto, producto_compuesto, producto_materiaprima, producto_envase
		...
		if (!borradoExitoso)
			return ERROR;
		//Volvemos a insertar el producto, especificando el id que tendrá, que es el mismo que tenía antes
		...
		if (compuesto.size() > 0)
			if (! (new GestionProductosHelper().setProductoCompuesto(idProducto, compuesto)))
				return ERROR;
		//Guardamos la fecha original de creación del producto
