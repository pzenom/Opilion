...
public class UpdaREFumiCantAction extends ActionSupport implements
	...
	public void setServletRequest(HttpServletRequest request) {
		System.out.println("UpdaREFumiCantAction Action...");
	...
	public String execute() throws Exception {	
		...
		for (int i = 0; i < cant.length;i++){
			...
			while (iter.hasNext()){
				...
				System.out.println("Añadiendo a la lista: "+ing);
		...
		for(
			Iterator it = mapaCantRE.keySet().iterator(); it.hasNext();) { 
		...
		request.getSession().setAttribute("envases",
				(new GestionDesgranadoHelper().updateRECantProceso(mapaCantRE,gprod.getOrden(),"Fumigado")));
		...
		new GestionUbicacionesHelper().sacarUbicaciones(ubicaciones, false, true, true);
		new GestionCongeladoHelper().guardarUbicacionesCongelado(ubicaciones, gprod.getOrden(), "Fumigado");
		return SUCCESS;
	}
