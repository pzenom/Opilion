...
public class OrdersParserBB implements IOrdersParserService {
	...
	public Vector orderParse(String fichero) throws Exception {
		...
		//La estructura está creada, pero ya está en el sistema?
	...
	//He aquí nuestro parser de líneas
	public Boolean parseador(String ficheroCompleto) {
		String [] temporal;
		String [] tokens;
		String [] validador;
		String nodoPadre=new String(); //para saber el ultimo nodo
		Long numeroDeLinea = 0L; //para llevar el registro de las líneas de productos
		...
		for (int i=0; i<temporal.length;i++) {
			...
			if(tokens[0].equalsIgnoreCase("UNH")){
				...
				if (tokens[3].equalsIgnoreCase("9")) {
					...
					pedido.setBgmFunc(tokens[3].trim());
				}else { mensaje="[ERROR][BMG] No es una función del Mensaje reconocida ";
					System.out.println("[ERROR][BMG] No es una función del Mensaje reconocida "); return true; }
			}else if(tokens[0].equalsIgnoreCase("DTM")){
				System.out.println("-----------DTM--------------------");
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[0]+"]");
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"]");
				validador=tokens[1].trim().split(":");
				//TODO: Faltaría meter el valor idOrden en la clase DTM
				...
				if (validador[0].equalsIgnoreCase("137")) {
					...
					System.out.println(">>>>>>>>>>>>>>>>["+validador[0]+"] Última Fecha/Hora de Entrega");
				...
				if (!tokens[1].equalsIgnoreCase("")) { 
					mensaje="[ERROR][ALI] Parámetro 1 no esperado";
					System.out.println("[ERROR][ALI] Parámetro 1 no esperado"); 
					return true; 
				} else if (!tokens[2].equalsIgnoreCase("")) { 
					mensaje="[ERROR][ALI] Parámetro 2 no esperado";
					System.out.println("[ERROR][ALI] Parámetro 2 no esperado"); 
					return true; 
					...
					mensaje="[ERROR][ALI] No es una Condición reconocida";
					System.out.println("[ERROR][ALI] No es una Condición reconocida"); return true; }
				pedido.setAliCond(tokens[3]);
			}else if(tokens[0].equalsIgnoreCase("FTX")){
				System.out.println("-----------FTX--------------------");
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[0]+"]");
				if (tokens[1].equalsIgnoreCase("DEL")) {
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Información de entrega");
				} else { 
					mensaje="[ERROR][ALI] No es un tipo de información reconocida";
					System.out.println("[ERROR][ALI] No es un tipo de información reconocida"); return true; }
				if (!tokens[2].equalsIgnoreCase("")) { 
					mensaje="[ERROR][FTX] Parámetro 2 no esperado";
					System.out.println("[ERROR][FTX] Parámetro 2 no esperado"); 
					return true; 
				} else if (!tokens[3].equalsIgnoreCase("")) { 
					mensaje="[ERROR][ALI] Parámetro 3 no esperado";
					System.out.println("[ERROR][ALI] Parámetro 3 no esperado"); 
					return true; 
				} else if (tokens[4].equalsIgnoreCase(""))  {
					mensaje="[ERROR][ALI] Parámetro 4 Vacío";
					System.out.println("[ERROR][ALI] Parámetro 4 Vacío"); 
					return true; 
				} else {
				...
				if (tokens[1].equalsIgnoreCase("MS")) {
					...
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Receptor de las Mercancías");
					...
					mensaje="[ERROR][NAD] No es un Formato de interlocutor válido ("+tokens[1]+")";
					System.out.println("[ERROR][NAD] No es un Formato de interlocutor válido ("+tokens[1]+")"); 
					return true;
				}
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[2]+"]");				
				System.out.println(">>>>>>>>>>>>>>>>["+validador[0]+"] Código de la empresa / Punto operacional EAN");
				if (!validador[1].equalsIgnoreCase("")) {
					mensaje="[ERROR][NAD] Parámetro 1 no esperado";
					System.out.println("[ERROR][NAD] Parámetro 1 no esperado");
					return true;
				}
				...
				if (validador[0].equalsIgnoreCase("API")) {
					...
					if (nodoPadre.equalsIgnoreCase("MS")) {
						...
						System.out.println(">>>>>>>>>>>>>>>>["+nodoPadre+"] Receptor de las Mercancías: "+validador[1]);
					...
					}else { System.out.println("[ERROR][RFF] No es un Formato de interlocutor Padre válido ("+tokens[1]+")"); return true; }
				...
				if (validador[0].equalsIgnoreCase("2")) {
					...
					if (validador[2].equalsIgnoreCase("9")) {
						...
						mensaje="[ERROR][CUX] No es una divisa de pedido válida ("+validador[2]+")";
						System.out.println("[ERROR][CUX] No es una divisa de pedido válida ("+validador[2]+")"); 
						return true; 
					}
				}else { 
					mensaje="[ERROR][CUX] No es una divisa de referencia válida ("+validador[0]+")";
					System.out.println("[ERROR][CUX] No es una divisa de referencia válida ("+validador[0]+")"); 
					return true; 
				}
				...
				if(tokens[1].equalsIgnoreCase(String.valueOf(numeroDeLinea))&&(!tokens[1].equalsIgnoreCase(""))) {
					//si hay mas líneas, inserta la anterior al array, sino, esperamos a que lo haga el resumen
					if(numeroDeLinea>1) {
					...
					lin = new OrdersLin();
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Numero de línea correcto");
					//numeroDeLinea++;
					...
					if(tokens[2].equalsIgnoreCase("")){
						...
						if (validador[1].equalsIgnoreCase("EN")) {
							...
							System.out.println(">>>>>>>>>>>>>>>>["+validador[0]+"] Articulo solicitado Número");
							System.out.println(">>>>>>>>>>>>>>>>["+validador[1]+"] Formato EAN");
						}else { 
							mensaje="[ERROR][LIN] Formato del artículo pedido diferente del número EAN13 ó EAN14 ("+validador[1]+")";
							System.out.println("[ERROR][LIN] Formato del artículo pedido diferente del número EAN13 ó EAN14 ("+validador[1]+")"); 
							return true; }
					}else { 
						mensaje="[ERROR][LIN] Parámetro 2 no esperado ("+tokens[2]+")";
						System.out.println("[ERROR][LIN] Parámetro 2 no esperado ("+tokens[2]+")"); 
						return true; }
				}else { 
					mensaje="[ERROR][LIN] Número de línea incorrecto/no secuencial ("+tokens[1]+")";
					System.out.println("[ERROR][LIN] Número de línea incorrecto/no secuencial ("+tokens[1]+")"); 
					return true; }
			}else if(tokens[0].equalsIgnoreCase("PIA")){
				System.out.println("-----------PIA--------------------");
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[0]+"]");
				if(!tokens[1].equalsIgnoreCase("")){
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Validador de identificación adicional ");
					validador=tokens[2].trim().split(":");
					System.out.println(">>>>>>>>>>>>>>>>["+validador[0]+"] Número de Articulo ");
					...
					mensaje="[ERROR][PIA] No hemos encontrado la función de identificación de producto. Esperado 1 ("+tokens[1]+")";
					System.out.println("[ERROR][PIA] No hemos encontrado la función de identificación de producto. Esperado 1 ("+tokens[1]+")"); 
					return true; }
			}else if(tokens[0].equalsIgnoreCase("IMD")){
				//TODO: Revisar este punto, puesto que debe necesitar cambios por los aspectos de Descripción color, talla, que muy seguramente Tierrina no se ve afectado
				...
				if (tokens[1].equalsIgnoreCase("F")) {
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Descripción del artículo en Forma Libre");
					...
					if(validador[0].equalsIgnoreCase("")&&validador[1].equalsIgnoreCase("")&&validador[2].equalsIgnoreCase("")
						...
						System.out.println(">>>>>>>>>>>>>>>>["+validador[3]+"] Descripción de producto");
						//lin.setImdDesc(validador[2]);
						imd.setImdDesc(validador[3]);
					}else { 
						mensaje="[ERROR][IMD] Descripción Libre no reconocida. ("+tokens[3]+")";
						System.out.println("[ERROR][IMD] Descripción Libre no reconocida. ("+tokens[3]+")"); 
						return true; }					
				} else if (tokens[1].equalsIgnoreCase("C")) {
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Descripción del artículo en Código");
					//no necesitamos destripar la tercera parte
					//imd.setImdDesc(tokens[3]);
					imd.setImdCodi(tokens[3]);
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[3]+"] El artículo es una unidad de consumo");
				} else { 
					mensaje="[ERROR][IMD] Descripción del artículo no conocida el formato. Esperado F/C ("+tokens[1]+")";
					System.out.println("[ERROR][IMD] Descripción del artículo no conocida el formato. Esperado F/C ("+tokens[1]+")"); 
					return true; }
				...
				if(tokens[2].equalsIgnoreCase("")){
					...
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[2]+"] Es una descripción de color ");
				} else if(tokens[2].equalsIgnoreCase("98")){
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[2]+"] Es una descripción de Talla ");
				} else { 
				...
				if (validador[0].equalsIgnoreCase("21")) {
					...
					if (!validador[1].equalsIgnoreCase("")) {
						...
						if(nodoPadre.equalsIgnoreCase("LOC")) {
							...
							System.out.println(">>>>>>>>>>>>>>>>["+validador[1]+"] Cantidad Pedida Asociada a una localización");
							nodoPadre = "";
							lin.setLoc(loc);
					...
					if (!validador[1].equalsIgnoreCase("")) {
						System.out.println(">>>>>>>>>>>>>>>>["+validador[1]+"] Numero de unidades de Consumo en la unidad de expedición");
				...
				//como una localizacion va acompañada de una qty...
				...
				if (validador[0].equalsIgnoreCase("203")) {
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"]");
					if (!validador[1].equalsIgnoreCase("")) {
						System.out.println(">>>>>>>>>>>>>>>>["+validador[1]+"] Importe Total Neto de la Línea de Artículo (incluye descuentos y cargos, no impuestos)");
				...
				if (validador[0].equalsIgnoreCase("AAA")) {
					...
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"]");
					if (!validador[1].equalsIgnoreCase("")) {
						System.out.println(">>>>>>>>>>>>>>>>["+validador[1]+"] Precio a título informativo (PVP) ");
						lin.setPriInf(validador[1]);
					} else { 
						mensaje="[ERROR][PRI] INF: Error en el Precio a título informativo (PVP) ("+validador[1]+")";
						System.out.println("[ERROR][PRI] INF: Error en el Precio a título informativo (PVP) ("+validador[1]+")"); 
						return true; }
				} else { 
				...
				System.out.println(">>>>>>>>>>>>>>>>["+String.valueOf(numeroDeLinea)+"] Para este número de línea");
				if(tokens[1].equalsIgnoreCase("7")){
					...
					if (validador[2].equalsIgnoreCase("9")) {
						...
						if (validador[1].equalsIgnoreCase("")&&!validador[0].equalsIgnoreCase("")) {
							System.out.println(">>>>>>>>>>>>>>>>["+validador[0]+"] Código de localización EAN formato n13 ");
							loc.setLoc(validador[0]);
						} else { 
							mensaje="[ERROR][LOC] Parametro excesivo o EAN vació ("+validador[1]+"|"+validador[0]+")";
							System.out.println("[ERROR][LOC] Parametro excesivo o EAN vació ("+validador[1]+"|"+validador[0]+")"); 
							return true; }
					} else { 
					...
					mensaje="[ERROR][LOC] Identificación de localización no reconocido ("+tokens[1]+")";
					System.out.println("[ERROR][LOC] Identificación de localización no reconocido ("+tokens[1]+")"); 
					return true; }
				// el loc lo guardamos en QTY
			}else if(tokens[0].equalsIgnoreCase("UNS")){
				System.out.println("-----------UNS--------------------");
				System.out.println(">>>>>>>>>>>>>>>>["+tokens[0]+"]");
				//metemos la última línea de artículo (o la única) en el array
				pedido.setLin(lin);
				...
				if(temporal.length == Integer.parseInt(tokens[1])) {
					System.out.println(">>>>>>>>>>>>>>>>["+tokens[1]+"] Las líneas de mensaje coinciden ["+temporal.length+"]");
					if(tokens[2].equalsIgnoreCase(pedido.getUnh())){
						//Guardamos el número de líneas, esto podríamos haberlo hecho al principio, hehehe
